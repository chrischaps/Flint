...
- **Post-processing pipeline** via `flint-render::postprocess` module — scene renders to `Rgba16Float` HDR intermediate buffer; bloom via progressive downsample/upsample chain (CoD:AW technique, 5-level mip chain with 13-tap downsample, 9-tap tent upsample, additive blend); composite fullscreen pass applies exposure → ACES tonemapping → gamma → vignette → sRGB surface; `PostProcessConfig` runtime-adjustable (enabled, exposure, bloom intensity/threshold, vignette intensity/smoothness); scene TOML `[post_process]` block for per-scene settings; CLI flags `--no-postprocess`, `--bloom-intensity`, `--bloom-threshold`, `--exposure`; F5 toggles bloom, F6 toggles entire post-processing pipeline; when post-processing active, PBR shaders output linear HDR (tonemapping disabled in fragment shader); legacy shader tonemapping path preserved as fallback when post-processing disabled
